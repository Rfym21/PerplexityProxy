 ...
 * 代理池状态处理函数
 */
func ProxyPoolStatusHandler(c *gin.Context) {
	...
	proxies := proxyPool.GetAllProxies()
	proxyInfos := proxyPool.GetProxyInfo()

	c.JSON(http.StatusOK, gin.H{
		"enabled":         true,
		"pool_size":       proxyPool.Size(),
		"configured_size": config.ConfigInstance.ProxyPoolSize,
		"api_url":         config.ConfigInstance.ProxyPoolAPI,
		"should_rotate":   proxyPool.ShouldRotate(),
		"proxies":         proxies,
		"proxy_details":   proxyInfos,
	})
}
 ...
 * 刷新代理池处理函数
 */
func RefreshProxyPoolHandler(c *gin.Context) {
	if !config.ConfigInstance.EnableProxyPool {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Proxy pool is disabled",
		})
		return
	}
