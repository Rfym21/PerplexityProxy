...
func NewClient(sessionToken string, proxyURL string, model string, openSerch bool) *Client {
	client := req.C().ImpersonateChrome().SetTimeout(time.Minute * 10)
	client.Transport.SetResponseHeaderTimeout(time.Second * 10)

	// 代理设置优先级：传入的代理 > 代理池 > 配置文件中的代理
	var finalProxy string
	if proxyURL != "" {
		finalProxy = proxyURL
	} else if config.ConfigInstance.EnableProxyPool {
		// 从代理池获取代理
		proxyPool := proxy.GetProxyPool()
		finalProxy = proxyPool.GetNextProxy()
		if finalProxy != "" {
			logger.Info(fmt.Sprintf("Using proxy from pool: %s", finalProxy))
		}
	} else if config.ConfigInstance.Proxy != "" {
		finalProxy = config.ConfigInstance.Proxy
	}

	if finalProxy != "" {
		client.SetProxyURL(finalProxy)
		logger.Info(fmt.Sprintf("Set proxy: %s", finalProxy))
	}
...
func (c *Client) SendMessage(message string, stream bool, is_incognito bool, gc *gin.Context) (int, error) {
	...
	if err != nil {
		...
		// 如果使用了代理池，处理代理错误
		if config.ConfigInstance.EnableProxyPool && c.currentProxy != "" {
			proxyPool := proxy.GetProxyPool()
			proxyPool.HandleProxyError(c.currentProxy, 500)
		}
	...
	if resp.StatusCode == 407 {
		logger.Error(fmt.Sprintf("Proxy authentication failed (407) for proxy: %s", c.currentProxy))
		if config.ConfigInstance.EnableProxyPool && c.currentProxy != "" {
			proxyPool := proxy.GetProxyPool()
			proxyPool.HandleProxyError(c.currentProxy, 407)
		}
		resp.Body.Close()
		return 407, fmt.Errorf("proxy authentication failed")
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		if config.ConfigInstance.EnableProxyPool && c.currentProxy != "" {
			proxyPool := proxy.GetProxyPool()
			proxyPool.HandleProxyError(c.currentProxy, resp.StatusCode)
		}
		resp.Body.Close()
