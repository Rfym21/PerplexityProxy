...
func LoadConfig() *Config {
	...
	// 解析代理池大小
	proxyPoolSize, err := strconv.Atoi(os.Getenv("PROXY_POOL_SIZE"))
	if err != nil {
		proxyPoolSize = 10 // 默认值
	}
	...
	config := &Config{
		...
		// 设置代理池API地址
		ProxyPoolAPI: os.Getenv("PROXY_POOL_API"),
		// 设置代理池大小
		ProxyPoolSize: proxyPoolSize,
		// 是否启用代理池
		EnableProxyPool: os.Getenv("ENABLE_PROXY_POOL") == "true",
		//是否匿名
...
func init() {
	...
	logger.Info(fmt.Sprintf("ProxyPoolAPI: %s", ConfigInstance.ProxyPoolAPI))
	logger.Info(fmt.Sprintf("ProxyPoolSize: %d", ConfigInstance.ProxyPoolSize))
	logger.Info(fmt.Sprintf("EnableProxyPool: %t", ConfigInstance.EnableProxyPool))
